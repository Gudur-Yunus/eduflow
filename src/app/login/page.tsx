    ...
    <PageTransition>
      ...
      >
        ...
        <form className="space-y-4" onSubmit={(e) => e.preventDefault()}>
          ...
          <div className="flex items-center justify-between text-sm py-2">
            <label className="flex items-center gap-2 text-[#6B7280] cursor-pointer">
              <input type="checkbox" className="rounded border-[#1F2937] bg-[#0A0D14] text-[#6366F1] focus:ring-[#6366F1]" />
              Remember me
            </label>
            <a href="#" className="text-[#6366F1] hover:text-[#A78BFA] transition-colors">Forgot password?</a>
          </div>
        ...
        <motion.button 
          whileHover={{ y: -2, scale: 1.02 }} 
          whileTap={{ y: 0, scale: 0.98 }}
          className="w-full bg-transparent border border-[#1F2937] text-white py-2.5 rounded-[8px] font-medium hover:bg-[#1F2937] transition-all flex items-center justify-center gap-2"
        >
        ...
        <p className="text-center text-[#6B7280] text-sm mt-8">
          Don&apos;t have an account? <Link href="/signup" className="text-[#6366F1] font-medium hover:text-[#A78BFA] transition-colors">Sign up</Link>
        </p>
        </motion.div>
      </div>
    </PageTransition>
